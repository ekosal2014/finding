package kh.com.finding.utils;

public class ConstsUtils {

	public static final String DEFAULT_ERROR_STATUS   = "9999";
	public static final String DEFAULT_SUCCESS_STATUS = "0000";
	
	/*message code register form*/
	public static final String NOTEMPTY_FULL_NAME = "NotEmpty.RegForm.full_name";
	public static final String NOTEMPTY_EMAIL     = "NotEmpty.RegForm.email";
	public static final String NOTVALID_EMAIL     = "NotValid.RegForm.email";
	public static final String DUPLICATED_EMAIL   = "Duplicated.RegForm.email";
	public static final String NOTEMPTY_PASSWORD  = "NotEmpty.RegForm.password";
	public static final String NOTEMPTY_CONFIRM_PASSWORD = "NotEmpty.RegForm.confirmpassword";
	public static final String NOTMATCH_CONFIRM_PASSWORD = "NotMatch.RegForm.confirmpassword";
	public static final String NOTEMPTY_CAPTCHA     = "NotEmpty.RegForm.captchaError";
	public static final String NOTEMPTY_REG_SUCCESS = "msg.reg.success";
	public static final String NOTEMPTY_USER_NAME   = "NotEmpty.RegForm.userName";
	public static final String DUPLICATED_USERNAME  = "Duplicated.RegForm.userName";
	
}
